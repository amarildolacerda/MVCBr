...
{ }
{ Para mais informa��es voc� pode fazer parte do grupo do Telegram }
{ }
...
begin
  ...
  try
    ...
    ckbFecharTortoise.Checked := ArqIni.ReadBool('CONFIG',
      'FecharTortoise', True);
...
procedure TfrmPrincipal.CopiarArquivosToLib;
  ...
  var
    ...
    begin
      ...
      begin
        ...
        CopyFile(PWideChar(AListArquivos[I]),
          PWideChar(IncludeTrailingPathDelimiter(sDirLibrary) + Arquivo), True);
      end;
    end;

  begin
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot),
      Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'VCL', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'FMX', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'helpers', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'oData', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'DMVC\sources', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'DMVC\lib\loggerpro', Extensao, TSearchOption.soAllDirectories);
    Mover(ListArquivos);
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
  ...
  procedure CopiasTemplates(Extensao: string);
  var
    ListArquivos: TStringDynArray;
    procedure MoverTemplates(var AListArquivos: TStringDynArray);
    var
      Arquivo: string;
      I: Integer;
    begin
      ...
      begin
        Arquivo := ExtractFileName(AListArquivos[I]);
        CopyFile(PWideChar(AListArquivos[I]),
          PWideChar(IncludeTrailingPathDelimiter(sDirTemplates) +
          Arquivo), True);
      end;
    end;
  ...
  begin
    ListArquivos := TDirectory.GetFiles(IncludeTrailingPathDelimiter(sDirRoot) +
      'templates', Extensao, TSearchOption.soAllDirectories);
    MoverTemplates(ListArquivos);
  end;
...
begin
  ...
  sDirTemplates := TPath.GetDocumentsPath +
    '\Embarcadero\Studio\code_templates\Delphi';
...
begin
  ...
  Sender.AddPathOption('U', oMVCBr.Installations[iVersion].LibFolderName
    [tPlatform]);
  Sender.AddPathOption('U', oMVCBr.Installations[iVersion].LibrarySearchPath
    [tPlatform]);
  Sender.AddPathOption('U', sDirLibrary);
  // -I<paths> = Include directories
  Sender.AddPathOption('I', oMVCBr.Installations[iVersion].LibrarySearchPath
    [tPlatform]);
  // -R<paths> = Resource directories
  Sender.AddPathOption('R', oMVCBr.Installations[iVersion].LibrarySearchPath
    [tPlatform]);
  ...
  begin
    ...
    if MatchText(VersionNumberStr, ['d17', 'd18', 'd19', 'd20', 'd21', 'd22',
      'd23', 'd24', 'd25']) then
      Sender.Options.Add
        ('-NSWinapi;Windows;System.Win;Data.Win;Datasnap.Win;Web.Win;Soap.Win;Xml.Win;Bde;System;Xml;Data;Datasnap;Web;Soap;Vcl;Vcl.Imaging;Vcl.Touch;Vcl.Samples;Vcl.Shell');
...
var
  ...
  procedure MostrarMensagemInstalado(const aMensagem: String;
    const aErro: String = '');
  var
    Msg: String;
  begin
    ...
    begin
      case sDestino of
        ...
        tdNone:
          Msg := 'Tipo de destino "nenhum" n�o aceito!';
      end;
    end
    else
    begin
...
begin
  ...
  try
    ...
    Cabecalho := 'Caminho: ' + edtDirDestino.Text + sLineBreak +
      'Vers�o do delphi: ' + edtDelphiVersion.Text + ' (' + IntToStr(iVersion) +
    ...
    WriteToTXT(AnsiString(PathArquivoLog),
      AnsiString('Setando par�metros de plataforma...'));
    ...
    Application.ProcessMessages;
    WriteToTXT(AnsiString(PathArquivoLog),
      AnsiString('Criando diret�rios de bibliotecas...'));
    ...
    Application.ProcessMessages;
    WriteToTXT(AnsiString(PathArquivoLog),
      AnsiString('Adicionando library paths...'));
    ...
    begin
      ...
      begin
        ...
        if oMVCBr.Installations[iVersion].CompilePackage
          (sDirPackage + NomePacote[iDpk], sDirLibrary, sDirLibrary) then
        begin
          lstMsgInstalacao.Items.Add
            (Format('Pacote "%s" compilado com sucesso.', [NomePacote[iDpk]]));
          lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
        end
        else
        begin
          Inc(FCountErros);
          lstMsgInstalacao.Items.Add(Format('Erro ao compilar o pacote "%s".',
            [NomePacote[iDpk]]));
          lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
        end;
      end;
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if oMVCBr.Installations[iVersion].InstallPackage
                (sDirPackage + NomePacote[iDpk], sDirLibrary, sDirLibrary) then
              begin
                lstMsgInstalacao.Items.Add
                  (Format('Pacote "%s" instalado com sucesso.',
                  [NomePacote[iDpk]]));
                lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
              end
              else
              begin
                Inc(FCountErros);
                lstMsgInstalacao.Items.Add
                  (Format('Ocorreu um erro ao instalar o pacote "%s".',
                  [NomePacote[iDpk]]));
                lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
              end;
            end;
      ...
      begin
        lstMsgInstalacao.Items.Add('');
        lstMsgInstalacao.Items.Add
          ('Abortando... Ocorreram erros na compila��o dos pacotes.');
        lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
      end;
    end
    else
    begin
      lstMsgInstalacao.Items.Add('');
      lstMsgInstalacao.Items.Add
        ('Para a plataforma de 64 bits os pacotes s�o somente compilados.');
      lstMsgInstalacao.ItemIndex := lstMsgInstalacao.Count - 1;
    end;
  ...
  begin
    Application.MessageBox
      ...
      'Clique em "Pr�ximo" para finalizar a instala��o.'), 'Instala��o',
      MB_ICONINFORMATION + MB_OK);
  end
  else
  begin
    if Application.MessageBox
      (PWideChar('Ocorreram erros durante o processo de instala��o, ' +
      sLineBreak + 'para maiores informa��es verifique o arquivo de log gerado.'
...
{$IFNDEF DEBUG}
  ...
  begin
    Stop := True;
    Application.MessageBox('Feche a IDE do delphi antes de continuar.',
...
begin
  ...
  begin
    Stop := True;
    Application.MessageBox('Clique no bot�o instalar antes de continuar.',
...
begin
  ...
  begin
    ...
    Application.MessageBox('Vers�o do delphi n�o suportada pelo projeto MVCBr.',
      'Erro.', MB_OK + MB_ICONERROR);
  end;
  ...
  begin
    ...
    Application.MessageBox('Diret�rio de instala��o n�o foi informado.',
      'Erro.', MB_OK + MB_ICONERROR);
  end;
  ...
  begin
    Stop := True;
    edtDelphiVersion.SetFocus;
    Application.MessageBox('Vers�o do delphi n�o foi informada.', 'Erro.',
      MB_OK + MB_ICONERROR);
  end;
  ...
  begin
    ...
    Application.MessageBox('Plataforma de compila��o n�o foi informada.',
      'Erro.', MB_OK + MB_ICONERROR);
  end;
...
begin
  ...
  begin
    // lblInfoObterFontes.Caption := 'Clique em "Atualizar" para efetuar a atualiza��o do reposit�rio MVCBr.';
    btnSVNCheckoutUpdate.Caption := 'Atualizar...';
    btnSVNCheckoutUpdate.Tag := -1;
  end
  else
  begin
    // lblInfoObterFontes.Caption := 'Clique em "Download" para efetuar o download do reposit�rio MVCBr.';
...
begin
  ...
  begin
    ...
    // checkout
    TSVN_Class.SVNTortoise_CheckOut(edtURL.Text + '/trunk', edtDirDestino.Text,
      ckbFecharTortoise.Checked);
  end
  else
  begin
    // update
    TSVN_Class.SVNTortoise_Update(edtDirDestino.Text,
      ckbFecharTortoise.Checked);
  end;
end;
...
begin
  if Application.MessageBox('Deseja realmente cancelar a instala��o?', 'Fechar',
