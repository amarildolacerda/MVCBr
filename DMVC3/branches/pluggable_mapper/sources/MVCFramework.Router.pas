...
begin
  ...
  ///  oData workaround to drop params from path info - by: amarildo lacerda
  ///  there are no other way to do !!!
  ///  sample or GET:  http://localhost:8080/OData/products(1)
  x := LRequestPathInfo.IndexOf('(');
  if x>0 then
    LRequestPathInfo := LRequestPathInfo.Remove(x) ;
