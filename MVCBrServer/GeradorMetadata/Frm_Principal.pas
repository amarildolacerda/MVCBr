...
type
  ...
  private
    ...
    procedure Conectar(Driver:string;Conn:TFDConnection;Server, Database, User, Pass, VendorLib:string; Port:Integer=0);
...
procedure TFrmPrincipal.Conectar(Driver:string;Conn:TFDConnection;Server, Database, User, Pass, VendorLib:string; Port: Integer=0);
begin
  ...
  if (Driver = 'Firebird') or ((Driver = 'Interbase')) then
  begin
     Conn.Params.Clear;
     Conn.DriverName := 'FB';
...
begin
  ...
  Metadata.Connection.GetTableNames('','','',lstTabelas.Items,[osMy],[tkTable]);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // ForeignKeys (continua a montagem do json adicionando as FKs)
          Metadata.Close;
          ...
          begin
            ...
            begin
              ...
              loRelation.AddPair('resource', Metadata.FieldByName('PKEY_TABLE_NAME').AsString);
              // relations.sourceKey: qual a coluna de recionamento no resource MASTER
              loRelation.AddPair('sourceKey', Fields.FieldByName('PKEY_COLUMN_NAME').AsString);
              // relations.targetKey: qual a coluna de relacionamento no resource DETAIL
              loRelation.AddPair('targetKey', Fields.FieldByName('COLUMN_NAME').AsString);
              // relations.join: utilizado para JOINs mais complexos ignorando “sourceKey” e “targetKey”
              loRelation.AddPair('join', '');
              Metadata.Next;
            end;
          end;
        end;
...
procedure  TFrmPrincipal.LoadPropertys(Index : Integer);
  ...
  begin
     ...
     _Propertys.Add('    property ' + _Campo + ': ' + NullableTipo + ' Index ' + IntToStr(_Index) + _ReadWrite);
  end;
...
begin
  ...
  begin
     ...
     _PropertysRelations.Add('    [Association(OneToOne,' + QuotedStr(Fields.FieldByName('COLUMN_NAME').AsString) + ',' + QuotedStr(Fields.FieldByName('PKEY_COLUMN_NAME').AsString) +')]');
     _PropertysRelations.Add('    property ' +Metadata.FieldByName('PKEY_TABLE_NAME').AsString + ': ' + 'T' + Metadata.FieldByName('PKEY_TABLE_NAME').AsString + ReadWrite);
  ...
  begin
    ...
    ReadWrite := ' read F' + Entidade.FieldList.Fields[i].FieldName + ' write F' + Entidade.FieldList.Fields[i].FieldName + ';';
    ...
    if Entidade.FieldByName(Campo).DataType in [ftBlob, ftMemo, ftWideMemo] then
       CreateBodyProperty(index,Campo,'String',ReadWrite,'taLeftJustify','''''',I)
    else
