...
# 
# Requirements:
# - JCL (JEDI Code Library) must be installed in Delphi
...
echo "NOTE: Requires JCL (JEDI Code Library) for JclIDEUtils unit"
cd "$INSTALL_DIR"
...
if wine "$DCC32_WIN" "MVCBrInstall.dpr" $CFLAGS $OUT_FLAGS -U"$UPATH" -I"$UPATH"; then
    ...
    if [ -f "$PROJECT_DIR/MVCBrInstall.exe" ]; then
        ...
        ls -lh "$PROJECT_DIR/MVCBrInstall.exe" 2>/dev/null || ls -lh "$PROJECT_DIR/MVCBrInstall.exe"
    else
    ...
    echo "Install JCL in Delphi IDE to build the installer"
    exit 1
fi